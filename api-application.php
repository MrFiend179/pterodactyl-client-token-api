...
/*
|--------------------------------------------------------------------------
| Client Token Generation (Admin Only)
|--------------------------------------------------------------------------
|
| Endpoint: /api/application/admin/generate-client-token
|
*/
Route::post('/admin/generate-client-token', function (Request $request) {
    ...
    // Optional: delete old keys if needed or check key count
    if ($user->apiKeys()->count() >= 25) {
        return response()->json(['error' => 'User has too many API keys'], 400);
    }

    // Create client token
    $token = $user->createToken('Auto-generated by admin', []); // [] = no IP restriction

    return response()->json([
        'identifier' => $token->accessToken->identifier,
        'secret_token' => $token->plainTextToken, // Only shown once
    ]);
});